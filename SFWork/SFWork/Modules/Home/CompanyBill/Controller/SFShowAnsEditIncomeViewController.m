//
//  SFShowAnsEditIncomeViewController.m
//  SFWork
//
...
@interface SFShowAnsEditIncomeViewController ()<UITableViewDelegate,UITableViewDataSource,SFExpenseCellDelegate,SFSuperSuborViewControllerDelagete,SFPhotoSelectCellDelegate,PickerToolDelegate,SFSelectPersonCellDelegate>
...
@implementation SFShowAnsEditIncomeViewController
...
- (void)initData {
    ...
    [self.dataArray addObjectsFromArray:[SFBillSearchModel shareShowItemArrayWithModel:_showModel]];
...
- (void)rightBtnAction:(UIButton *)sender{
    ...
    for (NSArray * array in self.dataArray) {
        for (SFBillSearchModel * model in array) {
            model.isClick = isCanEdit;
            if (model.persons.count>0) {
                for (SFSearchApprpvalModel * appModel in model.persons) {
                    appModel.isClick = isCanEdit;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.type == 4) {
        ...
        [cell cellImage:nil withIsEdit:!_isEdit withCmodel:nil withArr:_showModel.photos];
        cell.delegate = self;
        return cell;
    }
...
- (void)topView{
    ...
    topTitleLB.text = [NSString stringWithFormat:@"该账款详情由%@于%@创建",_showModel.listerName,_showModel.bizDate];
