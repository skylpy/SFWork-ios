...
@interface SFPhotoSelectCell ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation SFPhotoSelectCell
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    if (self.dataSource.count >= 5||self.isEdit) {
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row < self.dataSource.count||self.isEdit) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == self.dataSource.count) {
        if (self.delegate&&[self.delegate respondsToSelector:@selector(cellClickUpload:)]) {
            
            [self.delegate cellClickUpload:self.dataSource];
        }
    }else{
        if (self.delegate&&[self.delegate respondsToSelector:@selector(openClickLook:withRow:withView:)]) {
            
            [self.delegate openClickLook:self.dataSource withRow:indexPath.row withView:cell];
        }
    }
