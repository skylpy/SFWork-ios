//
//  SFJournalSetWeekViewController.m
//  SFWork
//
...
#import "SFJournalSetWeekViewController.h"
...
@interface SFJournalSetWeekViewController ()<UITableViewDelegate,UITableViewDataSource,DateTimePickerViewDelegate,SFPickerViewDelegate,SFSelectWeekViewDelegate>
...
@implementation SFJournalSetWeekViewController
...
- (void)requestData:(BOOL)isOn with:(BOOL)isO{
    ...
    [self.dataArray addObjectsFromArray: [SFJournalModel shareJournalSetWeekModel:isOn with:isO withM:self.smodel]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.type == 6) {
        ...
        [cell setTextChange:^(NSString * _Nonnull text) {
            @strongify(self)
            [SFJournalSetModel shareManager].contentOfWeekly = text;
        }];
        return cell;
    }
    if (model.type == 1 || model.type == 4) {
        ...
        [cell setSelectClick:^(BOOL isOn) {
            ...
            if (model.type == 1) {
                ...
                self.smodel.isRemindOfWeekly = isOn;
                if (isOn) {
                    [self requestData:YES with:YES];
                }else{
...
- (void)weekClick {
    ...
    customVC.getPickerValue = ^(NSString *compoentString, NSString *titileString) {
        ...
        if (self.model.type == 2) {
            ...
            self.smodel.weeklyStartDate = compoentString;
            self.smodel.weeklyStartTime = titileString;
        }
        if (self.model.type == 3) {
            self.model.destitle = [NSString stringWithFormat:@"%@   %@",compoentString,titileString];
            self.smodel.weeklyEndDate = compoentString;
            self.smodel.weeklyEndTime = titileString;
        }
        [self.tableView reloadData];
    };
...
- (void)savaJournalSetData{
    ...
    [dict setValue:@"WEEKLY" forKey:@"dailyType"];
    ...
    [SFJournalHttpModel updateDailySettings:dict success:^{
        ...
        [MBProgressHUD showSuccessMessage:@"周报设置成功"];
    } failure:^(NSError * _Nonnull error) {
        
        [MBProgressHUD showErrorMessage:@"周报设置失败"];
    }];
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectText:(NSString *)text withRow:(NSInteger)row withSelectIndex:(NSInteger)selectIndex{
    ...
    if ([text isEqualToString:@"提前1小时提醒"]) {
        self.smodel.remindTimeOfWeekly = @"1";
    }
    if ([text isEqualToString:@"提前2小时提醒"]) {
        self.smodel.remindTimeOfWeekly = @"2";
    }
    if ([text isEqualToString:@"提前4小时提醒"]) {
        self.smodel.remindTimeOfWeekly = @"4";
    }
    if ([text isEqualToString:@"提前8小时提醒"]) {
        self.smodel.remindTimeOfWeekly = @"8";
    }
}
