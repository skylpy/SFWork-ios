...
@interface SFSuperSuborViewController ()<UITableViewDelegate,UITableViewDataSource,SFEmployeeCellDelegate>
...
@implementation SFSuperSuborViewController
...
- (UIButton *)rightButton{
    ...
    if (!_rightButton) {
        ...
        [[_rightButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            if (self.delagete && [self.delagete respondsToSelector:@selector(multiplesSelectEmoloyee:)]) {
                
                [self.delagete multiplesSelectEmoloyee:self.selectArray];
            }
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = self.isSubor ? @"直属下属":@"直属上司";
    [self initDrawUI];
    if (self.isSubor) {
        [self getEmployees];
    }else{
...
- (void)getAdmin{
    ...
    [SFOrganizationModel getDirectlyAdminsSuccess:^(NSArray<SFEmployeesModel *> * _Nonnull list) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.delagete && [self.delagete respondsToSelector:@selector(singlesSelectEmoloyee:)]) {
        
        [self.delagete singlesSelectEmoloyee:model];
    }
    [self.navigationController popViewControllerAnimated:YES];
}
