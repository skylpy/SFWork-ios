//
//  SFTemplateMonthViewController.m
//  SFWork
//
...
@interface SFTemplateMonthViewController ()<UITableViewDelegate,UITableViewDataSource,DateTimePickerViewDelegate,SFPickerViewDelegate,SFSelectWeekViewDelegate>
...
@implementation SFTemplateMonthViewController
...
- (void)requestData:(BOOL)isOn with:(BOOL)isO{
    ...
    [self.dataArray addObjectsFromArray: [SFJournalModel shareReportSetMonthModel:isOn with:isO withM:self.smodel]];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (section == 1) {
        ...
        self.titleLabel.text = [NSString stringWithFormat:@"本月%@日 %@:00开始填写当天的汇报，本月%@日 %@:00前提交为正常提交，之后提交为迟交",self.smodel.startDate,self.smodel.startTime,self.smodel.endDate,self.smodel.endTime];
...
- (void)weekClick {
    ...
    customVC.getPickerValue = ^(NSString *compoentString, NSString *titileString) {
        ...
        if (self.model.type == 2) {
            ...
            self.smodel.startTime = titileString;
        }
        if (self.model.type == 3) {
            self.model.destitle = [NSString stringWithFormat:@"%@号   %@",compoentString,titileString];
            self.smodel.endDate = compoentString;
            self.smodel.endTime = titileString;
        }
        [self.tableView reloadData];
    };
...
- (void)savaJournalSetData{
    ...
    [dict setValue:@"MONTH" forKey:@"period"];
